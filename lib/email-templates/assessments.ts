...
export function buildAssessmentInvitationEmailHtml(
  url: string,
  ...
  const contentHtml = `
    <p style="font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.6;color:#3F3F46;margin:0 0 16px 0;">
      Hello ${escapeHtml(first)}, you have been invited to take this assessment for ${escapeHtml(APP_NAME)}. The link below is yours alone and works once.
    </p>
    <p style="font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.6;color:#3F3F46;margin:0 0 16px 0;">
  ...
  return renderProtocolEmail({
    previewText: `You've been invited to take "${assessmentTitle}"`,
...
export type AssessmentCompletedEmailParams = {
  url: string;
  assessmentTitle: string;
...
export function buildAssessmentCompletedEmailHtml(params: AssessmentCompletedEmailParams): string {
  ...
  return renderProtocolEmail({
    previewText: `${params.candidateName} completed ${params.assessmentTitle}: ${params.percent}%`,
    heading: params.assessmentTitle,
    contentHtml,
    buttonText: "Review full response in Admin",
