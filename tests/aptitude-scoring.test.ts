...
import { scoreResponse, unansweredRequired, type ScorableQuestion } from "@/lib/modules/aptitude/scoring";

/**
 * Pure, so the rules that decide what a candidate's result says about them
 * are testable exhaustively — the aptitude equivalent of Assessments'
 * scoring tests (and, further back, the attendance projection). A wrong
 * number here is not a bug anyone spots; it is a wrong conclusion about a
 * candidate, feeding a hiring decision.
 */
...
const multi = (id: string, points = 2): ScorableQuestion => ({
  id,
  kind: AptitudeQuestionKind.MULTI_CHOICE,
  points,
  options: [
    { id: `${id}_a`, isCorrect: true },
...
const free = (id: string): ScorableQuestion => ({
  id,
  kind: AptitudeQuestionKind.FREE_TEXT,
  points: 5,
  options: [],
});
...
describe("one correct answer", () => {
  ...
  it("awards nothing when a correct option is chosen alongside a wrong one", () => {
    const result = scoreResponse([single("q1")], [{ questionId: "q1", selectedOptionIds: ["q1_a", "q1_b"] }]);
    expect(result.scoredPoints).toBe(0);
  });
...
describe("several correct answers", () => {
  it("awards the points only for the exact set", () => {
    const result = scoreResponse([multi("q1")], [{ questionId: "q1", selectedOptionIds: ["q1_a", "q1_b"] }]);
    expect(result.scoredPoints).toBe(2);
  });

  it("does not accept order as a difference", () => {
    const result = scoreResponse([multi("q1")], [{ questionId: "q1", selectedOptionIds: ["q1_b", "q1_a"] }]);
    expect(result.scoredPoints).toBe(2);
  });
  ...
  it("awards nothing for selecting everything", () => {
    const result = scoreResponse([multi("q1")], [{ questionId: "q1", selectedOptionIds: ["q1_a", "q1_b", "q1_c"] }]);
    expect(result.scoredPoints).toBe(0);
  });

  it("ignores a duplicated selection rather than miscounting it", () => {
    const result = scoreResponse([multi("q1")], [{ questionId: "q1", selectedOptionIds: ["q1_a", "q1_a", "q1_b"] }]);
    expect(result.scoredPoints).toBe(2);
  });
});
...
describe("written answers", () => {
  ...
  it("record whether anything was written, for a human to read", () => {
    const result = scoreResponse([free("q1")], [{ questionId: "q1", selectedOptionIds: [], text: "  " }]);
    expect(result.perQuestion[0]!.answered).toBe(false);
...
describe("odd inputs that must not produce an odd score", () => {
  it("ignores option ids belonging to another question", () => {
    const result = scoreResponse([single("q1")], [{ questionId: "q1", selectedOptionIds: ["q9_a"] }]);
    expect(result.scoredPoints).toBe(0);
  });
  ...
  it("clamps negative points to zero", () => {
    const result = scoreResponse([single("q1", -5)], [{ questionId: "q1", selectedOptionIds: ["q1_a"] }]);
    expect(result.scoredPoints).toBe(0);
...
describe("required questions", () => {
  ...
  it("lists the ones with nothing selected", () => {
    expect(
      unansweredRequired([required(single("q1")), required(single("q2"))], [
        { questionId: "q1", selectedOptionIds: ["q1_a"] },
      ]),
    ).toEqual(["q2"]);
  });
  ...
  it("counts whitespace in a written answer as unanswered", () => {
    expect(unansweredRequired([required(free("q1"))], [{ questionId: "q1", selectedOptionIds: [], text: "   " }])).toEqual([
      "q1",
    ]);
  });
