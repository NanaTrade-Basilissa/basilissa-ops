...
export function AssessmentPublicLinkDialog({
  action,
  linkUrl,
  values,
  trigger,
  ...
  const [state, formAction, isPending] = useActionState<AssessmentFormState, FormData>(action, undefined);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Allow takers to self-administer the assessment via a single shareable link.
          </DialogDescription>
        </DialogHeader>
        ...
        <form action={formAction} className="space-y-4 w-full min-w-0">
          <div className="flex items-start gap-3 rounded-lg border border-border p-3.5 w-full min-w-0">
            <Switch
              id="assessmentPublicLinkEnabled"
              name="enabled"
              checked={enabled}
              onChange={(e) => setEnabled(e.target.checked)}
            />
            <div className="space-y-1 min-w-0 flex-1">
              <Label htmlFor="assessmentPublicLinkEnabled" className="font-medium cursor-pointer">
                Enable public link access
              </Label>
              <p className="text-xs text-muted-foreground">
                Anyone with the link can take this assessment. Each person gets their own attempt.
              </p>
            </div>
          </div>
            ...
            <div className="space-y-3 rounded-lg border border-border bg-muted/20 p-3.5 w-full min-w-0">
              ...
              <div className="grid gap-3 sm:grid-cols-2 pt-1 w-full min-w-0">
                <div className="space-y-1.5 min-w-0">
                  <Label htmlFor="assessmentPublicNameMode">Participant name</Label>
                  <NativeSelect
                    id="assessmentPublicNameMode"
                    name="nameMode"
                    value={nameMode}
                ...
                <div className="space-y-1.5 min-w-0">
                  <Label htmlFor="assessmentPublicEmailMode">Participant email</Label>
                  <NativeSelect
                    id="assessmentPublicEmailMode"
                    name="emailMode"
                    value={emailMode}
              ...
              <p className="text-[11px] text-muted-foreground">
                Configure whether takers must identify themselves or take the assessment anonymously.
              </p>
            </div>
          )}
