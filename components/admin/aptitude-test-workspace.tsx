...
export function AptitudeTestWorkspace({
  ...
  return (
    <div>
      <nav aria-label="Aptitude test sections" className="flex gap-1 overflow-x-auto border-b border-border">
        {TABS.map((t) => {
          const Icon = t.icon;
          ...
          <div className="space-y-6">
            {!isPublished ? (
              <p className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
                Publish this test to start inviting candidates and collecting responses.
              </p>
            ) : (
              <>
